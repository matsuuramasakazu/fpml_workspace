...
def test_adjust_date_modified_conventions():
    ...
    assert calendar.adjust_date(sun_may_end, "MODFOLLOWING", "GBLO") == date(2026, 5, 29)
    
    # 2026-05-24 は日曜日 (中旬)
    # FOLLOWING だと 2026-05-25 (月曜日) で同じ5月のため、
    # MODFOLLOWING でも 2026-05-25 になるべき
    sun_may_mid = date(2026, 5, 24)
    assert calendar.adjust_date(sun_may_mid, "MODFOLLOWING", "GBLO") == date(2026, 5, 25)
    ...
    assert calendar.adjust_date(sat_aug_start, "MODPRECEDING", "GBLO") == date(2026, 8, 3)
