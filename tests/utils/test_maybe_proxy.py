...
def test_maybe_proxy_success():
    ...
    maybe = MaybeProxy(trade)

    # 正常な属性およびインデックスアクセス
    assert maybe.swap.value == swap
    assert maybe.swap.swap_stream.value == [stream]
    assert maybe.swap.swap_stream[0].value == stream
    assert maybe.swap.swap_stream[0].calculation_period_amount.value == amount
    assert maybe.swap.swap_stream[0].calculation_period_amount.currency.value == "JPY"
    assert maybe.swap.swap_stream[0].calculation_period_amount.amount.value == 1000000.0

    # 正常系では失敗情報はNoneまたは空
    assert maybe.swap.swap_stream[0].calculation_period_amount.failed_at is None
    assert maybe.swap.swap_stream[0].calculation_period_amount.last_valid_object is None


def test_maybe_proxy_none_middle():
    # 途中の要素がNoneである場合
    swap = Swap(swap_stream=None)  # swap_streamがNone
    trade = Trade(swap=swap)

    maybe = MaybeProxy(trade)
    endpoint = maybe.swap.swap_stream[0].calculation_period_amount.currency
...
def test_maybe_proxy_index_out_of_range():
    # リストが空でインデックス範囲外になる場合
    swap = Swap(swap_stream=[])  # 空リスト
    trade = Trade(swap=swap)

    maybe = MaybeProxy(trade)
    endpoint = maybe.swap.swap_stream[0].calculation_period_amount
...
def test_maybe_proxy_attribute_error():
...
def test_maybe_proxy_type_error_for_non_indexable():
...
def test_maybe_proxy_root_is_none():
    # ルートオブジェクト自体がNoneの場合
    maybe = MaybeProxy(None)
    endpoint = maybe.swap.swap_stream[0]
